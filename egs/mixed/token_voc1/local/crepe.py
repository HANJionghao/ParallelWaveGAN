...
class BasePitchExtractor:
    ...
    def post_process(self, x, sampling_rate, f0, pad_to):
        ...
        if f0.shape[0] <= 0:
            return (
                np.zeros(pad_to, dtype=np.float32),
                vuv_vector.cpu().numpy(),
            )
        if f0.shape[0] == 1:
            return (
                np.ones(pad_to, dtype=np.float32) * f0[0],
                vuv_vector.cpu().numpy(),
            )
...
class CrepePitchExtractor(BasePitchExtractor):
    def __init__(
        ...
        print(f"Setting up crepe with f0_min: {f0_min}, f0_max: {f0_max}, periodicity_threshold: {periodicity_threshold}, silence_threshold: {silence_threshold}, keep_zeros: {keep_zeros}, device: {device}, model: {model}, use_fast_filters: {use_fast_filters}, decoder: {decoder}")
        if decoder == "viterbi":
    ...
    def __call__(self, x, sampling_rate=44100, pad_to=None):
        ...
        pd = torchcrepe.threshold.Silence(self.silence_threshold)(pd, x, sampling_rate, self.hop_length)
        f0 = torchcrepe.threshold.At(self.periodicity_threshold)(f0, pd)
